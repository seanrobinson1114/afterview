...
function render() {
		...
		if((x >= 0) && (x < width)) {
			if((y >= 0) && (y < height)) {
				if(p.z < 0) {
					...
					drawPointWithGradient(ctx, x, y, 10, "rgb(50,200,190)", 0.5);
				}
			}
		}
	}
	ctx.restore();
	ctx.fillStyle = "rgb(235,225,55)";
	// ctx.fillText("di Guido D'Albore", width-90, height-5);
	rotation += Math.PI/180.0;
  console.log( 'SHRINK', shrink, distance );

	if(distance < 1000 && shrink === false) {
		distance += 10;
	}

  else if( distance >= 1000 && shrink === false)
    shrink = true;
  ...
  else if( distance === 500 && shrink === true )
    shrink = false;
